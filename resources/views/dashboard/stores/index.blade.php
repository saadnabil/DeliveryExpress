...
@section('content')
    <div class="row">
        <div class="col-6">
            <h6 class="mb-0 text-uppercase">{{ __('translation.Stores') }} </h6>
        </div>
        @can('store-create')
        <div class="col-6">
                ...
                <table id="example2" class="table">
                    <thead class="table-light">
                        <tr >
                            ...
                            <th>{{ __('translation.Store Name') }}</th>
                            ...
                            <tr style="line-height: 40px !important;">
                                ...
                                <td>
                                    <div class="d-flex order-actions">
                                        @can('store-edit')
                                        <a href="{{ route('stores.edit', $row) }}" class="ms-3"><i
                                                class="bx bxs-edit"></i></a>
                                        @endcan
                                        @can('store-delete')
                                        <a href="{{ route('stores.destroy', $row) }}" class="ms-3 confirm-delete"><i
                                                class="bx bxs-trash"></i></a>
                                        <form method="POST" action="{{ route('stores.destroy', $row)}}">
                                            @csrf
                                            @method('delete')
                                        </form>
                                        @endcan
