...
@section('content')
    <div class="card">
        <div class="card-body p-4">
            <h5 class="card-title">{{ __('translation.Add New Question') }}</h5>
            <hr />
            <form class="form-body mt-4" method="post" action="{{ route('faqs.store') }}" enctype="multipart/form-data">
                {{ csrf_field() }}
                <div class="row">
                    <div class="col-lg-8">
                        <div class="border border-3 p-4 rounded">
                            <div class="mb-3">
                                ...
                                <input required name="question" value="{{ old('question') }}" type="text"
                            ...
                            <div class="mb-3">
                                ...
                                <textarea required name="answer" class="form-control  @error('answer') error-input @enderror" id="input11" rows="3">{{ old('answer') }}</textarea>
                                @error('answer')
                                    <div class="error-validation">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select  requierd name="application"
                                    ...
                                    <option value="deliveryApplication">{{ __('translation.deliveryApplication') }}
                                    </option>
                                    <option value="storeApplication">{{ __('translation.storeApplication') }}</option>
                                </select>
                                @error('application')
                                    <div class="error-validation">{{ $message }}</div>
                                @enderror
